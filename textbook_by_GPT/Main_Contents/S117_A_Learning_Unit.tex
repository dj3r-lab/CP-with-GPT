...
% Stage 117 S117-A
% =========================================================
\part{Stage 117 --- Advanced Structures}
...
\begin{checklistbox}[복습 체크리스트]
\begin{itemize}
\item ordinary DSU와 Rollback DSU 중 어떤 구조가 필요한지 문제 조건으로 구분할 수 있는가?
\item rollback이 필요한 구현에서 path compression을 일반적으로 제외하는 이유를 설명할 수 있는가?
\item union by size를 사용했을 때 path compression 없이도 \code{find()}가 $O(\log N)$인 이유를 설명할 수 있는가?
\item successful union 하나를 되돌리기 위해 어떤 정보를 history에 저장해야 하는지 설명할 수 있는가?
\item \code{history.size()}를 snapshot token으로 사용하는 원리를 설명할 수 있는가?
\item 중첩된 snapshot/transaction에서 변경을 LIFO 순서로 복원해야 하는 이유를 설명할 수 있는가?
\item connectivity 외에 component size, component count, aggregate 값을 함께 관리할 때 어떤 상태를 rollback 대상에 포함해야 하는지 판단할 수 있는가?
\item 전체 상태 복사와 change logging의 시간·공간 trade-off를 비교할 수 있는가?
\end{itemize}
\end{checklistbox}
